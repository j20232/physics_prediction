...
class BaseTest(object):
    ...
    def __init__(self, sess, outputs, args, seed):
        self.sess = sess
        self.args = args
        self.seed = seed
    ...
    def get_ranges(self, batch):
        ...
        isone = np.concatenate(
            ([0], np.equal(use_frame, 1).view(np.int8), [0]))
    ...
    def unroll_noninit(self, frame, batch, use_ground_truth_for_unroll):
        ...
        _ap_full_particle = np.take(
            ...
            axis=-3)
        if frame < len(batch['full_particles']):
            self.true_particles.append(
                self._get_copy(frame, _ap_full_particle))
