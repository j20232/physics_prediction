    ...
    def get_plot_data(particles, index=index, coloring=coloring):
        ...
        if coloring is 'colormap':
            if isinstance(index, list):
                assert len(
                    index) == 2, 'provide start and end of your slice only!'
    ...
    for curr_suboffset, particles in zip([0, n_frame], all_particles):
        for curr_frame in xrange(n_frame):
            axes = fig.add_subplot(
                *[num_col, n_frame, curr_frame+1+curr_suboffset], projection='3d')
            ...
            def animate(i):
                x, y, z, c, minimum, maximum = get_plot_data(
...
def correct_videos(videos):
    videos = videos.replace(
        'loop', 'loop style="display:block; margin: 0 auto;"')
    return videos
...
def vis_true_and_pred(path_true, path_pred, ex=3,
    ...
    if static_path is not None:
        ...
        min_all -= 0.25 * dist
        max_all += 0.25 * dist
        idx = np.logical_and.reduce([static_particles[:, 0] >= min_all[0], static_particles[:, 0] <=
                                     max_all[0], static_particles[:, 2] >= min_all[2], static_particles[:, 2] <= max_all[2]])
        temp_particles = static_particles[idx]
        temp_particles = np.tile(np.expand_dims(np.concatenate([temp_particles, np.zeros(
            (temp_particles.shape[0], particles.shape[2]-temp_particles.shape[1]))], axis=1), 0), [particles.shape[0], 1, 1])
        temp_particles[:, :, 14] = 2
        particles = np.concatenate([temp_particles, particles], axis=1)
    if not just_fig:
        video = create_video(particles, coloring='colormap', index=[
                             14, 15], angle=angle, elev=elev, s=s)
    ...
    if not just_fig:
        ...
        min_all = np.minimum(
            np.min(particles[:, :, 0:3], axis=(0, 1)), min_all)
        max_all = np.maximum(
            np.max(gt_particles[:, :, 0:3], axis=(0, 1)), max_all)
        create_figures([gt_particles, particles], coloring='colormap', index=[14, 15], angle=angle,
                       elev=elev, s=s, min_all=min_all, max_all=max_all, pdf_path=pdf_path, gap_frame=gap_frame)
