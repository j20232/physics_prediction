...
def pad_to_tensors(ret_dict,
    ...
    for key, value in ret_dict.items():
        ...
        for curr_value in value:
            curr_shape = curr_value.shape
            pad_shape = [(0, m_shape - c_shape)
                         for m_shape, c_shape in zip(max_shape,   curr_shape)]
...
def get_group_cached_ts(group_file):
    ...
    for each_group_file in group_file:
        ...
        ret_dict = update_cached_dict(
            ret_dict, 'kNN_valid_flag', kNN_valid_flag)

        ret_dict = update_cached_dict(
            ret_dict, 'mult_mat', all_data['mult_mat'])
        ...
        ret_dict = update_cached_dict(
            ret_dict, 'num_p_rep', all_data['num_p_rep'])
        ret_dict = update_cached_dict(
            ret_dict, 'grav_flag', all_data['grav_flag'])
        ...
        ret_dict = update_cached_dict(
            ret_dict, 'drt_father_list', drt_father_list)

        no, no_wo_group = all_data['mult_mat'].shape
        no_wo_group_flag = np.arange(no) < no_wo_group
        ret_dict = update_cached_dict(
            ret_dict, 'no_wo_group_flag', no_wo_group_flag)
