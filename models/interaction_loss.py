...
def get_velocity_loss(outputs, norm_loss=1, pred_particle_velocity=None,
                      gt_particle_velocity=None, str_type="", sl=1, use_running_mean=False):
    ...
    if gt_particle_velocity is None:
        gt_particle_velocity = sparse_particles[:, -1, :, 15: 18]

    valid_particles_mask = tf.not_equal(
        sparse_particles[:, -1, :, 14], 0)
    ...
    if norm_loss == 1:
        ...
        for curr_depth in xrange(max_depth):
            ...
            curr_vel_loss = (
                (curr_gt_next_vel_pts - curr_pred_next_vel_pts) / mean_l2norm_vel) ** 2
...
def get_preserve_distance_loss(outputs,
    ...
    if avd_obj_mask == 0:
        ...
        obj1_mask = tf.ones(
            outputs['sparse_particles'][:, -1, :, 14:15].shape, dtype=tf.float32)
        obj2_mask = tf.ones(
...
def flex_l2_particle_loss(
    ...
    if preserve_distance_radius > 0:
        ...
        retval['preserve_distance_loss'] = tf.reduce_mean(
            preserve_distance_loss)
